...
**Without this library** — wiring Stripe.js by hand takes ~40 lines even for a basic card form:
...
<script>
  // 1. Load Stripe and create an Elements instance
  const stripe = Stripe('pk_test_xxxxx');
  const elements = stripe.elements({
    clientSecret: 'pi_xxxxx_secret_xxxxx',
  });

  // 2. Mount the card UI element
  const cardElement = elements.create('card');
  cardElement.mount('#card-element');

  // 3. Forward validation errors to the user
  cardElement.on('change', ({ error }) => {
    const display = document.getElementById('error-message');
    display.textContent = error ? error.message : '';
  });

  // 4. Handle submit: disable button, confirm payment, handle result
...
**With stripe-pwa-elements** — one CDN line and a single tag handle all of the above:

```html
<!-- Drop in the CDN loader once per page -->
<script type="module" src="https://unpkg.com/stripe-pwa-elements/dist/wpkyoto/stripe-pwa-elements.esm.js"></script>

<!-- Declare the component — Stripe initialises, mounts, and submits automatically -->
<stripe-payment-element
  publishable-key="pk_test_xxxxx"
  intent-client-secret="pi_xxxxx_secret_xxxxx"
></stripe-payment-element>
...
### Why teams choose this library

- **Framework-agnostic** — Standard Web Components that work with React, Vue, Angular, Svelte, or plain HTML with no adapter needed.
- **No build step required** — one CDN `<script>` line is enough to get started.
- **PCI DSS SAQ-A eligible** — card data stays inside Stripe-hosted iframes; your page never touches raw card numbers (see [Security & PCI DSS SAQ-A](#security--pci-dss-saq-a) below).
- **Production-proven** — powers the Web layer of [`@capacitor-community/stripe`](https://github.com/capacitor-community/stripe), which receives **4,000+ weekly downloads**.
